...
export function LeadCaptureScreen() {
  ...
  const isExistingClient = !!(prefillName || prefillPhone);
    ...
    <div className="min-h-screen bg-background px-5 pt-14">
      <div className="max-w-[600px] mx-auto">
        ...
        <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="text-[34px] font-bold mb-8 text-foreground tracking-tight">
          {isExistingClient ? 'New Project' : 'New Lead'}
        </motion.h1>
        {isExistingClient && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="-mt-6 mb-6 text-[15px] text-muted-foreground">
            for {prefillName}
          </motion.p>
        )}
